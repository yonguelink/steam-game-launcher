    ...
    internal static class Launcher
    {
        ...
        private static void Main(string[] args)
        {
            ...
            catch(ArgumentException ex)
            {
                ...
                Console.Error.WriteLine("Press enter to quit...");
                Console.ReadLine();
            }
            catch (Exception err) when (attempt++ < maxAttemps)
            {
                Console.WriteLine($"Something happened {err}, retrying... ({attempt/maxAttemps})");
                Main(args);
            }
            catch (Exception err) {
        ...
        private static void StartGame(string steamInstallPath, string steamGameId, bool asAdmin)
        {
            ...
            if (steamInstallPath.Equals("none", StringComparison.CurrentCultureIgnoreCase))
            {
